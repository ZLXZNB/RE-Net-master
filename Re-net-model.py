...
class RENet(nn.Module):
    def __init__(self, in_dim, h_dim, num_rels, dropout=0, model=0, seq_len=10, num_k=10):
        ...
        #        self.encoder = make_model(4 * h_dim, h_dim)
        #        self.encoder_r = make_model(3 * h_dim, h_dim)
        self.encoder = nn.GRU(4 * h_dim, h_dim, batch_first=True)
        self.encoder_r = nn.GRU(4 * h_dim, h_dim, batch_first=True)
    ...
    def forward(self, triplets, s_hist, o_hist, graph_dict, subject=True):
        ...
        #        s_packed_input = s_packed_input.data
        #        s_packed_input_r = s_packed_input_r.data
        print("s_packed_input")
        print(s_packed_input.data.shape)
        #        src_mask = (s_packed_input != pad).unsqueeze(-2)
        #        print("src_mask")
        #        print(src_mask)
        ...
        ###### Relations
        #        s_r_mask =  (s_packed_input_r != pad).unsqueeze(-2)
        tt, s_q = self.encoder_r(s_packed_input_r)
        print("tt")
        print(tt.data.shape)
        print("s_q")
        print(s_q.data.shape)
        s_q = s_q.squeeze()
